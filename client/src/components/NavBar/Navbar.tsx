...
function NavBar() {
  ...
  return (
    <nav className="relative flex items-center justify-between p-5 bg-amber-500 gap-20">
      ...
      <BsBackpackFill className="text-9xl sm:text-8xl md:text-7xl lg:text-5xl xl:text-5xl text-white hover:text-black"/>
      <FaCircleUser className="text-9xl sm:text-8xl md:text-7xl lg:text-5xl xl:text-5xl text-white hover:text-black" onClick={() => setIsProfileModalOpen(true)}/>

      <div className={`fixed inset-0 bg-black bg-opacity-50 ${isProfileModalOpen ? 'block' : 'hidden'}`} onClick={() => setIsProfileModalOpen(false)}>
 <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-amber p-4 rounded text-center">
    <a href="/" className="block text-white px-2 hover:text-amber-500 transform hover:scale-110 transition-transform duration-200">Your reservations</a>
    <a href="/" className="block text-white px-2 hover:text-amber-500 transform hover:scale-110 transition-transform duration-200">Your products</a>
    <a href="/" className="block text-white px-2 hover:text-amber-500 transform hover:scale-110 transition-transform duration-200">Sign out</a>
 </div>
</div>
